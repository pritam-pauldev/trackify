...
const submitBtn = document.querySelector("#submitSigninBtn");
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    if (status === 401) {
      ...
      showMessage("error", "No account found with this email. Please sign up first.");
    } else if (status === 400) {
      showMessage("error", serverMsg || "Invalid details. Please check your inputs and try again.");
    } else if (status === 429) {
      showMessage("error", "Too many attempts. Please wait a moment and try again.");
    } else if (!err.response) {
      showMessage("error", "Unable to reach the server. Please check your connection.");
    } else {
      showMessage("error", serverMsg || "Sign in failed. Please try again later.");
    }
} finally {
    setLoading(false);
  }
